...
## 1. Project Overview
- manager_qa: quản lý Q&A giữa học viên và trung tâm.
- Mục đích: giúp người dùng xem được câu hỏi đã được trả lời bởi trung tâm
...
- Entity mở rộng:
  ...
  - `QuestionContext`, 
    ...
    - `content`, `string`
    - `answer`, `string`
    - `id_question_type`, `int`, `FK Questiontype id`
    - `date`, `LocalDate`, `default`
    - `status`, `Enum(WAITING, RESPONSE)`
...
- Stack: 
  - Backend : Spring Boot 4.0.6, Java 17, Lombok, Spring Data JPA, 
  - Frontend : Boostrap 5, thymeleaf
